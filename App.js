...
export default function App() {
  ...
  useEffect(() => {
    // Fullscreen on Android (hide bottom nav bar)
  ...
  const calculate = () => {
    ...
    const totalRate = (growth - 1) * 100;
    setTotalReturn(totalRate.toFixed(2) + "%");

    const p = parseFloat(principal);
    if (!isNaN(p)) {
      const result = p * growth;
      setTotalAmount(new Intl.NumberFormat("ko-KR").format(result) + "원");
    } else {
      setTotalAmount(null);
    }
  };
